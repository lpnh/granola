...
#[recipe(name = InsRecipe, content = Cow<'static, str>)]
...
impl<R: InsRecipe> HasInsAttrs for HtmlIns<R> {
    fn ins_attrs_mut(&mut self) -> &mut InsAttrs {
...
macro_rules! ins {
    ...
    (@recipe $($r:ty),+) => {
        $crate::html::HtmlIns::<$crate::cookbook!($($r),+)>::from_recipe()
    };
    (@recipe $($r:ty),+ ; $content:expr $(,)?) => {
        $crate::html::HtmlIns::<$crate::cookbook!($($r),+)>::new($content)
    };
    (@recipe $($r:ty),+ ; $first:expr $(, $rest:expr)+ $(,)?) => {
        $crate::html::HtmlIns::<$crate::cookbook!($($r),+)>::new($crate::bake_block![$first $(, $rest)*])
    };
    (@recipe $($r:ty),+ ; @newline $content:expr $(,)?) => {
        $crate::html::HtmlIns::<$crate::cookbook!($($r),+)>::new($crate::bake_newline!($content))
    };
    (@recipe $($r:ty),+ ; @inline $($content:expr),+ $(,)?) => {
        $crate::html::HtmlIns::<$crate::cookbook!($($r),+)>::new($crate::bake_inline![$($content),+])
    };
}
