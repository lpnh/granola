...
macro_rules! samp {
    ...
    (@cookbook $r:ty $(,)?) => {
        $crate::html::HtmlSamp::<$r>::from_cookbook()
    };
    (@cookbook $r:ty ; $content:expr $(,)?) => {
        $crate::html::HtmlSamp::<$r>::from_cookbook().content($content)
    };
    (@cookbook $r:ty ; $first:expr $(, $rest:expr)+ $(,)?) => {
        $crate::html::HtmlSamp::<$r>::from_cookbook().content($crate::bake![$first $(, $rest)*])
    };
}
